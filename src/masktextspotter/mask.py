...
class SegmentationMask(object):
    ...
    def __getitem__(self, item):
        if isinstance(item, (int, slice)):
            ...
            selected_polygons = []
            if isinstance(item, Tensor) and item.dtype == mindspore.bool_:
                item = item.nonzero()
                item = item.squeeze(1) if item.prod() > 0 else item
                item = item.asnumpy().tolist()
            for i in item:
...
class Polygons(object):
    ...
    def rotate(self, angle, r_c, start_h, start_w):
        poly = self.polygons[0].asnumpy().reshape(-1, 2)
        poly[:, 0] += start_w
        poly[:, 1] += start_h
    ...
    def convert(self, mode="mask"):
        ...
        if mode == "mask":
            try:
                ...
                mask = ops.ones((height, width), dtype=mindspore.uint8)
                return mask
...
class CharPolygons(object):
    ...
    def __init__(
        ...
        if isinstance(char_boxes, CharPolygons):
            ...
            if char_classes is None:
                char_classes = [
                    Tensor(p[8], dtype=mindspore.float32) for p in char_boxes
                ]
            char_boxes = [
                Tensor(p[:8], dtype=mindspore.float32) for p in char_boxes
            ]
    ...
    def convert(self, mode="char_mask"):
        ...
        if mode == "char_mask":
            ...
            return Tensor(char_map), Tensor(char_map_weight)
            ...
            return (
                Tensor(char_map),
                Tensor(char_map_weight),
                Tensor(decoder_target),
                Tensor(word_target),
            )
